...
namespace aspect::script {
	namespace {
		...
		script_registry& registry() {
			//NOTE: put this static variable in function so can make sure it is initialized before being accessed.
	...
	namespace detail {
		...
		script_creator get_script_creator(size_t tag) {
			auto script = aspect::script::registry().find(tag);
			assert(script != aspect::script::registry().end() && script->first == tag);
...
LPSAFEARRAY get_script_names() {
	const u32 size{ (u32)aspect::script::script_names().size() };
	if (!size) return nullptr;
	CComSafeArray<BSTR> names(size);
	for (u32 i{ 0 }; i < size; i++)
	{
		names.SetAt(i, A2BSTR_EX(aspect::script::script_names()[i].c_str()), false);
	}
	return names.Detach();
}
#endif //USE_WITH_EDITOR