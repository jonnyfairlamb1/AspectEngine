...
namespace aspect::platform {
	...
	namespace {
		...
		/////////////////////////////////////////////////////////////////
		// TODO: this part will be handled by a free list container
		...
		void resize_window(const window_info& info, const RECT& area) {
			//adjust the window size for current device
		...
		void resize_window(window_id id, u32 width, u32 height) {
			...
			//update the internal data when we are hosting the window in the level editor
			if (info.style & WS_CHILD) {
				GetClientRect(info.hwnd, &info.client_area);
			}else{
				//check if its fullscreen incase the user has changed their screen resolution.
		...
		void set_window_fullscreen(window_id id, bool is_fullscreen) {
			...
			if (info.is_fullscreen != is_fullscreen) {
				...
				if (is_fullscreen) {
					//store the current window dimensions so they can be restored later
					GetClientRect(info.hwnd, &info.client_area);
					RECT rect;
	...
	window create_window(const window_init_info* const init_info) {
			...
			wc.lpszClassName = L"AspectWindow";
			...
			//adjust the window for device size
			AdjustWindowRect(&rect, info.style, FALSE);

			const wchar_t* caption{ (init_info && init_info->caption) ? init_info->caption : L"Aspect Game" };
			...
			info.hwnd = CreateWindowEx(
				...
				left,
				top,				//initial window position
				width,
				height,				//initial window dimensions
...
#else
#error "must implement at least on platform."
#endif //_WIN64

	void window::set_fullscreen(bool is_fullscren) const {
		assert(is_valid());
		set_window_fullscreen(_id, is_fullscren);
	}
