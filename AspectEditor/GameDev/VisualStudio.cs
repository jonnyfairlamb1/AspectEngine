            ...
            if (_vsInstance == null) {
                ...
                while (monikerTable.Next(1, currentMoniker, IntPtr.Zero) == 0) {
                    ...
                    if (name.Contains(_progID)) {
                        ...
                        if (hResult < 0 || obj == null) throw new COMException($"Running object tables GetObject() returned HRESULT: {hResult:X8}");
    ...
    public static void BuildSolution(Project project, string configName, bool showWindow = true) {
        ...
        for (int i = 0; i < 3 && !BuildDone; i++) {
            try {
                ...
                try {
                    foreach (var pdbFile in Directory.GetFiles(Path.Combine($"{project.ProjectPath}", $@"x64\{configName}"), "*.pdb")) {
                        File.Delete(pdbFile);
                    }
                ...
                Debug.WriteLine(ex.Message);
                Debug.WriteLine($"Attempt {i}: failed to build {project.ProjectName}");
                Thread.Sleep(1000);
            }
        }
    }
