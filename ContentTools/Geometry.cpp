...
namespace aspect::tools {
	namespace {
		...
		void recalculate_normals(mesh& m) {
			const u32 num_indeces{ (u32)m.raw_indices.size() };
			m.normals.resize(num_indeces);

			for (u32 i{ 0 }; i < num_indeces; ++i) {
					...
					if (!is_hard_edge) {
						for (u32 k{ j + 1 }; k < num_refs; ++k) {
							...
							if (!is_soft_edge) {
								...
								//NOTE: were accounting for the length of n1 in this calculation because it can possibly change in this loop iteration.
		...
		void pack_vertices_static(mesh& m) {
			...
			assert(num_vertices);
			m.packet_vertex_static.reserve(num_vertices);
			...
			for (u32 i{ 0 }; i < num_vertices; ++i) {
				...
				m.packet_vertex_static.emplace_back(packed_vertex::vertex_static{
		...
		u64 get_mesh_size(const mesh& m) {
			...
			const u64 size{
				su32 + m.name.size() +	//mesh name length
				su32 +					// lod id
				su32 +					// vertex size
		...
		u64 get_scene_size(const scene& scene) {
			...
			for (auto& lod : scene.lod_groups) {
				u64 lod_size{
					su32 + lod.name.size() + //lod name length and room for LPD string
		...
		void pack_mesh_data(const mesh& m, u8* const buffer, u64& at) {
			...
			memcpy(&buffer[at], m.packet_vertex_static.data(), s); at += s;

			// room for indeces
			s = vertex_size * num_indices;
