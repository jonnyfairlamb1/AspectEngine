...
namespace aspect::tools {
	namespace {
		...
		mesh create_plane(const primitive_init_info& info, u32 horizontal_index = axis::x, u32 vertical_index = axis::z, bool flip_winding = false, v3 offset = { -0.5f, 0.f, -0.5f },
			v2 u_range = { 0.f, 1.f }, v2 v_range = { 0.f,1.f }) {
			...
			const u32 num_indeces{ 3 * 2 * horizontal_count * vertical_count };
			assert(m.raw_indices.size() == num_indeces);
